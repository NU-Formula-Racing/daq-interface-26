...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
--
-- Generated on: 2025-08-14 04:05:10
...
SELECT _postgis_package_object('FUNCTION', '_st_clip (
	rast raster, nband integer[],
	geom geometry,
	nodataval float8[] , crop boolean , touched boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_clip (
	rast raster,
	nband integer[],
	geom geometry,
	nodataval float8[],
	crop boolean , touched boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_clip (
	rast raster, nband integer,
	geom geometry,
	nodataval float8, crop boolean , touched boolean )');
...
SELECT _postgis_package_object('FUNCTION', 'st_clip (
	rast raster,
	geom geometry,
	nodataval double precision, crop boolean , touched boolean )');
SELECT _postgis_package_object('FUNCTION', 'st_clip (
	rast raster,
	geom geometry,
	crop boolean, touched boolean )');
...
--
-- UPGRADE SCRIPT TO PostGIS 3.5.3
--
...
CREATE OR REPLACE FUNCTION postgis_raster_scripts_installed() RETURNS text
       AS $$ SELECT trim('3.5.3'::text || $rev$ aab5f55 $rev$) AS version $$
...
CREATE OR REPLACE FUNCTION st_band(rast raster, nbands text, delimiter char DEFAULT ',')
	RETURNS RASTER
	AS $$ SELECT  @extschema@.ST_band($1, pg_catalog.regexp_split_to_array(pg_catalog.regexp_replace($2, '[[:space:]]', '', 'g'), E'\\' || pg_catalog.array_to_string(pg_catalog.regexp_split_to_array($3, ''), E'\\'))::int[]) $$
	LANGUAGE 'sql' IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE OR REPLACE FUNCTION st_mapalgebra(
	...
	RETURNS raster
	AS $$
	select @extschema@._ST_mapalgebra(ARRAY[ROW($1,$2)]::@extschema@.rastbandarg[],$3,$6,NULL::integer,NULL::integer,$7,$8,$4,$5,VARIADIC $9)
	$$ LANGUAGE 'sql' IMMUTABLE PARALLEL SAFE;
CREATE OR REPLACE FUNCTION _st_mapalgebra(
	rastbandargset rastbandarg[],
	expression text,
...
CREATE OR REPLACE FUNCTION st_value(rast raster, x integer, y integer, exclude_nodata_value boolean DEFAULT TRUE)
    RETURNS float8
    AS $$ SELECT @extschema@.st_value($1, 1::integer, $2, $3, $4) $$
...
CREATE OR REPLACE FUNCTION st_pixelaspoint(rast raster, x integer, y integer)
	RETURNS geometry
	AS $$ SELECT @extschema@.ST_PointN(@extschema@.ST_ExteriorRing(geom), 1) FROM @extschema@._ST_pixelaspolygons($1, NULL, $2, $3) $$
	LANGUAGE 'sql' IMMUTABLE STRICT PARALLEL SAFE;
		...
		ELSE
			...
			RETURN QUERY
				SELECT
					emptygeom,
					NULL::float8
				FROM @extschema@.ST_GeomCollFromText('GEOMETRYCOLLECTION EMPTY', @extschema@.ST_SRID($1)) emptygeom;
		END IF;
	END;
	$$
	LANGUAGE 'plpgsql' IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE OR REPLACE FUNCTION ST_Intersection(
	...
	BEGIN
		...
		newnodata1 := coalesce(nodataval[1], @extschema@.ST_BandNodataValue(rast1, band1), @extschema@.ST_MinPossibleValue(@extschema@.ST_BandPixelType(rast1, band1)));
		newnodata2 := coalesce(nodataval[2], @extschema@.ST_BandNodataValue(rast2, band2), @extschema@.ST_MinPossibleValue(@extschema@.ST_BandPixelType(rast2, band2)));
...
END
$postgis_proc_upgrade$;
-- Rename _st_clip ( raster, integer[], geometry, float8[], boolean ) deprecated in PostGIS 305, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = '_st_clip(raster, integer[], geometry, float8[], boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _st_clip(raster, integer[], geometry, float8[], boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function _st_clip(raster, integer[], geometry, float8[], boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION _st_clip( raster, integer[], geometry, float8[], boolean ) RENAME TO _st_clip_deprecated_by_postgis_305;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function _st_clip(raster, integer[], geometry, float8[], boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function _st_clip(raster, integer[], geometry, float8[], boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION _st_clip(
	rast raster, nband integer[],
	geom geometry,
	nodataval float8[] DEFAULT NULL, crop boolean DEFAULT TRUE, touched boolean DEFAULT false
)
	RETURNS raster
	AS '$libdir/postgis_raster-3', 'RASTER_clip'
	LANGUAGE 'c' IMMUTABLE PARALLEL SAFE;
-- Rename st_clip ( raster, integer[], geometry, float8[], boolean ) deprecated in PostGIS 305, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_clip(raster, integer[], geometry, float8[], boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, integer[], geometry, float8[], boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_clip(raster, integer[], geometry, float8[], boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_clip( raster, integer[], geometry, float8[], boolean ) RENAME TO st_clip_deprecated_by_postgis_305;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, integer[], geometry, float8[], boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_clip(raster, integer[], geometry, float8[], boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION st_clip(
	...
	geom geometry,
	nodataval float8[],
	crop boolean DEFAULT true, touched boolean DEFAULT false
)
	RETURNS raster
 	AS $$
	BEGIN
		...
		RETURN @extschema@._ST_Clip(rast, nband, geom, nodataval, crop, touched);
	END;
	$$ LANGUAGE 'plpgsql' IMMUTABLE PARALLEL SAFE;
-- Rename st_clip ( raster, integer, geometry, float8, boolean ) deprecated in PostGIS 305, if needed
DO LANGUAGE 'plpgsql'
...
CREATE OR REPLACE FUNCTION st_clip(
	rast raster, nband integer,
	geom geometry,
	nodataval float8, crop boolean DEFAULT true, touched boolean DEFAULT false
)
	RETURNS raster AS
	$$ SELECT @extschema@.ST_Clip(rast, ARRAY[nband]::integer[], geom, ARRAY[nodataval]::float8[], crop, touched) $$
	LANGUAGE 'sql' IMMUTABLE PARALLEL SAFE;
...
END;
$postgis_proc_upgrade$;
-- Rename st_clip ( raster, integer, geometry, boolean ) deprecated in PostGIS 305, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_clip(raster, integer, geometry, boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, integer, geometry, boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_clip(raster, integer, geometry, boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_clip( raster, integer, geometry, boolean ) RENAME TO st_clip_deprecated_by_postgis_305;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, integer, geometry, boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_clip(raster, integer, geometry, boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION st_clip(
	...
	RETURNS raster AS
	$$ SELECT @extschema@.ST_Clip(rast, ARRAY[nband]::integer[], geom, null::double precision[], crop, touched) $$
	LANGUAGE 'sql' IMMUTABLE PARALLEL SAFE;
-- Rename st_clip ( raster, geometry, float8[], boolean ) deprecated in PostGIS 305, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_clip(raster, geometry, float8[], boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, geometry, float8[], boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_clip(raster, geometry, float8[], boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_clip( raster, geometry, float8[], boolean ) RENAME TO st_clip_deprecated_by_postgis_305;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, geometry, float8[], boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_clip(raster, geometry, float8[], boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION st_clip(
	...
	RETURNS raster AS
	$$ SELECT @extschema@.ST_Clip(rast, NULL, geom, nodataval, crop, touched) $$
	LANGUAGE 'sql' IMMUTABLE PARALLEL SAFE;
-- Rename st_clip ( raster, geometry, float8, boolean ) deprecated in PostGIS 305, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
...
CREATE OR REPLACE FUNCTION st_clip(
	rast raster,
	geom geometry,
	nodataval double precision, crop boolean DEFAULT true, touched boolean DEFAULT false
)
	RETURNS raster AS
	$$ SELECT @extschema@.ST_Clip(rast, NULL, geom, ARRAY[nodataval]::float8[], crop, touched) $$
	LANGUAGE 'sql' IMMUTABLE PARALLEL SAFE;
-- Rename st_clip ( raster, geometry, boolean ) deprecated in PostGIS 305, if needed
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
DECLARE
    detail TEXT;
...
BEGIN
    ...
    BEGIN
        ...
        SELECT proargnames
        FROM pg_catalog.pg_proc
        WHERE oid = 'st_clip(raster, geometry, boolean)'::regprocedure
        INTO argnames;


    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, geometry, boolean) does not exist';
        RETURN; -- nothing to do
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Checking if replaced function st_clip(raster, geometry, boolean) exists got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
    ...
    -- The renamed function will eventually be drop.
    BEGIN
        ALTER FUNCTION st_clip( raster, geometry, boolean ) RENAME TO st_clip_deprecated_by_postgis_305;
    EXCEPTION
    WHEN undefined_function THEN
        RAISE DEBUG 'Replaced function st_clip(raster, geometry, boolean) does not exist';
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS detail := PG_EXCEPTION_DETAIL;
        RAISE EXCEPTION 'Attempting to rename replaced function st_clip(raster, geometry, boolean) got % (%)', SQLERRM, SQLSTATE
            USING DETAIL = detail;
    END;
...
CREATE OR REPLACE FUNCTION st_clip(
	rast raster,
	geom geometry,
	crop boolean, touched boolean DEFAULT false
)
	RETURNS raster AS
	$$ SELECT @extschema@.ST_Clip(rast, NULL, geom, null::float8[], crop, touched) $$
	LANGUAGE 'sql' IMMUTABLE PARALLEL SAFE;
